...
//change vivek partyall
function partyall() {

    var APIURL1 = localStorage.getItem("APIURL");
    //APIURL = GBCServicePath + "/Values/updateUseraccount";
    var APIURL = GBCServicePath + "/Values/Getallparty";
    var url = GBCServicePath + "/Values/Getallparty";
    {
        $.ajax({
            url: url,
            ...
            success: function (data) {
                response($.map(data, function (item, id) {
                    if (item.PNAME.indexOf("$") == 0) {
                        ...
                        localStorage.setItem("GBCUserID", item.PNAME);
...
function changeAccount() {
    ...
    //$(".show-page-loading-msg").click();
    // update_user();
    var previoushrcode = localStorage.getItem("LoginUser");
    $("#up_msg").text("YOUR CURRENT USER NAME  CANNOT BE CHANGED");
    $("#txtuser").val(previoushrcode);
}
//Account change
function update_user() {
    var previoushrcode = localStorage.getItem("LoginUser");
    $("#up_msg").text("YOUR CURRENT USER NAME  CANNOT BE CHANGED");
    //var changehrcode = $("#txtuser").val().trim();
    $("#txtuser").text(previoushrcode);

    // changehrcode = changehrcode.toUpperCase();
    var changePassword = $("#txtpass").val().trim();
    if (changePassword == "") {
    ...
    if (changePassword != "") {
        var APIURL1 = localStorage.getItem("APIURL");
        //APIURL = GBCServicePath + "/Values/updateUseraccount";
        APIURL = APIURL1 + "/Connection/updateUseraccount";
        $.ajax(
            {
                url: APIURL,
                data: { hrcode: previoushrcode, Password: changePassword },
                type: "GET",
                dataType: "json",
                processData: true,
                ...
                success: function (data) {
                    if (data != "") {
                        $("#up_msg").text("Updated Succefully...");
                        localStorage.setItem("LoginUser", changehrcode);
                        localStorage.setItem("LoginUserPass", changePassword);
                        $("#up_msg").text("UPDATED SUCCEFULLY PLEASE LOGOUT AND LOGIN WITH NEW PASSWORD");
                    }
...
function fun_showCart1() {
    fun_AddItemInCart();
    $("#info").hide();
    window.location.href = "#Item-cart1";
...
function SetItem_Count() {
    ...
    if (arr != null) {
        ...
        if (Count < 1) {
            ...
            $("#P2").text("");
            $("#Label7t").text("");//cart-total-amt
            $("#netamt").text("");
            $("#grsamt").text("");
            $("#cart-tblamt").hide();//cart-tbl-amt
            $("#Empty").show();
            //$("#lbl-cart-cnt").hide();
            //$("#lbl-cart-cnt1").hide();
            ...
            $("#P2").text("Total " + Number(arrLength - 1) + " Item found");
            ...
            $("#detail_cart").text("Total Item : " + Number(splitArr.length - 1));
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data != "") {
                ...
                $("#cart-tblamt").show(); 
                $("#grsamt").text(Number(data.split('|')[0]).toFixed(2));
                $("#netamt").text(Number(data.split('|')[1]).toFixed(2));
                $("#cart-totalamt").text("");
            }
        },
        error: function (xmlHttpRequest, textStatus, errorThrown) {
